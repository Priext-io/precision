...
interface BondEscrow {
    function depositProposerBond(bytes32 marketID, uint256 amount) external;
    function releaseProposerBond(bytes32 marketID) external returns (uint256);
    function depositChallengerBond(bytes32 marketID, uint256 amount) external;
    function slashAndTransfer(bytes32 marketID, address winner, uint256 slashAmount) external returns (uint256);
}

interface PCSAlignmentGuard {
...
contract FinalityEngine is ReentrancyGuard, Pausable, AccessControl {
    ...
    BondEscrow public bondEscrow;  // ✅ Removed immutable
    ...
    constructor(address _bondEscrow, address _pcsGuard, address pauser) {
        // ✅ Only validate PCS guard, bondEscrow can be set later
        require(_pcsGuard != address(0), "Invalid PCS guard");
        pcsGuard = PCSAlignmentGuard(_pcsGuard);
        _grantRole(PAUSER_ROLE, pauser);
    }

    // ✅ NEW: Setter function (only admin, only once)
    function setBondEscrow(address _bondEscrow) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(bondEscrow) == address(0), "Already set");
        require(_bondEscrow != address(0), "Invalid address");
